...
describe('FacebookOAuthClient', () => {
  ...
  describe('buildAuthorizeUrl', () => {
    it('builds an authorize URL with the app id, Page scopes, redirect_uri, and state', () => {
      ...
      expect(url.searchParams.get('redirect_uri')).toBe(
        'http://localhost:3001/social/facebook/callback',
      );
  ...
  describe('fetchAccountInfo', () => {
    it('picks the first Facebook Page the user manages', async () => {
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: async () => ({
    ...
    it('re-exchanges the stored long-lived user token and re-derives a fresh Page token', async () => {
      const fetchMock = jest
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            data: [{ id: 'page-1', name: 'My Page', access_token: 'fresh-page-token' }],
          }),
        });
      global.fetch = fetchMock as unknown as typeof fetch;
